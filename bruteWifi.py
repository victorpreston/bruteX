...
def help():
    print(f"\t{r}-> {c}Usage:~ {y}python3 bruteWifi.py <wordlist>\n\t{r}-> {c}python3 bruteWifi.py {y}(it wil use default wordlist")
    exit()
...
sprint(f"\n\n{r} Note: {c}This tool is made by Xpert for educational purpose...")
sprint(f"\n{g}Preparing Attack...")
time.sleep(2)
clear()
banner()
...
tried = [x.strip("\n").split("--")[0] for x in open("avail_wifis.txt","r").readlines()]
found = [x.strip("\n") for x in open("tried_passwords.txt","r").readlines()]
ts = 15
...
def test(i ,face,x,key,ts):
    ...
    for n,password in enumerate(key):
        if wifi_name+"--"+password in found:
            ...
            with open("tried_password" , "a") as f:
